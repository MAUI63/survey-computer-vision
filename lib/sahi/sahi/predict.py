...
def get_prediction(
    ...
    num_dummy_images_at_end_of_batch = 0
    if required_batch_size is not None:
        if len(images) < required_batch_size:
            print("Padding batch with dummy images")
            blank = np.zeros_like(images[0])
            num_dummy_images_at_end_of_batch = required_batch_size - len(images)
            images += [blank] * num_dummy_images_at_end_of_batch
    detection_model.perform_inference(images)
    time_end = time.time() - time_start
    durations_in_seconds["prediction"] = time_end

    # Remove the last dummy images from the batch ... man this is gross = )
    if num_dummy_images_at_end_of_batch > 0:
        detection_model._original_predictions = detection_model._original_predictions[
            :-num_dummy_images_at_end_of_batch
        ]
    ...
    object_prediction_lists: List[ObjectPrediction] = detection_model.object_prediction_list_per_image
    # postprocess matching predictions
    if postprocess is not None:
        object_prediction_lists = [
            postprocess(object_prediction_list) for object_prediction_list in object_prediction_lists
        ]
    ...
    if verbose >= 1:
        ...
        print(
            "Post-processing performed in",
            durations_in_seconds["postprocess"],
            "seconds.",
        )

    results = []
    for img, object_prediction_list in zip(images, object_prediction_lists):
        results.append(
            PredictionResult(
                image=img, object_prediction_list=object_prediction_list, durations_in_seconds=durations_in_seconds
            )
        )
    return results
...
def get_sliced_prediction(
    ...
    num_group = int(math.ceil(num_slices / batch_size))
    ...
    for group_ind in range(num_group):
        ...
        for i in range(batch_size):
            img_idx = group_ind * batch_size + i
            if img_idx >= num_slices:
                break
            image_list.append(slice_image_result.images[img_idx])
            shift_amount_list.append(slice_image_result.starting_pixels[img_idx])
        ...
        prediction_results = get_prediction(
            ...
            verbose=0,
            required_batch_size=batch_size if pad_batches else None,
        )
